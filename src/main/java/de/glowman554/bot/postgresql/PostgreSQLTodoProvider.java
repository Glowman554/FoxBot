...
public class PostgreSQLTodoProvider extends TodoProvider {
    private final Connection connection;

    public PostgreSQLTodoProvider(Connection connection) {
        this.connection = connection;

        try {
            PreparedStatement role = connection.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS todo (userId text NOT NULL, todoId INT NOT NULL, todo TEXT NOT NULL, done INT NOT NULL, PRIMARY KEY (userId, todoId))");
            role.execute();
            role.close();
    ...
    public void deleteTodo(String userId, int todoId) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("delete from todo where userId = ? and todoId = ?");
    ...
    public void changeTodoDone(String userId, int todoId, boolean done) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("update todo set done = ? where userId = ? and todoId = ?");
    ...
    public void createTodo(String userId, String todo) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("insert into todo (userId, todoId, todo, done) values (?, ?, ?, 0)");
            statement.setString(1, userId);
    ...
    public HashMap<Integer, Pair<Boolean, String>> loadTodo(String userId) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("select todoId, done, todo from todo where userId = ?");
            statement.setString(1, userId);
            ResultSet resultSet = statement.executeQuery();

            HashMap<Integer, Pair<Boolean, String>> result = new HashMap<>();
            while (resultSet.next()) {
                result.put(resultSet.getInt("todoId"),
                        new Pair<>(resultSet.getInt("done") == 1, resultSet.getString("todo")));
            }
            resultSet.close();
            statement.close();
