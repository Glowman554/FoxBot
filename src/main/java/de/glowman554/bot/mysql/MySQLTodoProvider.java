...
public class MySQLTodoProvider extends TodoProvider {
    private final Connection connection;

    public MySQLTodoProvider(Connection connection) {
        this.connection = connection;

        try {
            PreparedStatement role = connection.prepareStatement("CREATE TABLE IF NOT EXISTS todo (userId varchar(100) NOT NULL, todoId INT NOT NULL, todo TEXT NOT NULL, done INT NOT NULL, PRIMARY KEY (userId, todoId))");
            role.execute();
            role.close();
